"""
train_model_v3.py — Stacked accuracy improvements over v2.

Stacked techniques (A + B + C + E):
  A. QAT   — Quantization-Aware Training (recovers INT8 loss)
  B. SE    — Squeeze-Excitation channel attention blocks
  C. Focal — Categorical focal loss (focuses on hard steep/speed classes)
  E. EMA   — Exponential Moving Average of weights

Still: 48×48×3 RGB input, INT8 output, Zephyr TFLM compatible.
Uses tf_keras (Keras 2) because tensorflow-model-optimization QAT
requires the legacy Keras backend.
"""
import os
import sys
import time
...
# QAT (tfmot) requires the legacy Keras 2 backend
...
BATCH_SIZE = 32
EPOCHS = 250          # float training
QAT_EPOCHS = 25       # QAT fine-tuning
INITIAL_LR = 1e-3
QAT_LR = 1e-4         # low LR for QAT fine-tune
LABEL_SMOOTHING = 0.1
MIXUP_ALPHA = 0.2
FOCAL_GAMMA = 2.0
EMA_DECAY = 0.999
...
HISTORY_PNG = os.path.join(MODEL_DIR, "training_history_v3.png")
...
# ═══════════════════════════════════════════════════════════════════════
#  2. MODEL ARCHITECTURE  —  DS-CNN + Squeeze-Excitation (B)
# ═══════════════════════════════════════════════════════════════════════
def _dw_sep_block(x, filters, stride=1):
    """Depthwise 3×3 → Pointwise 1×1 → BN → ReLU  (proven v2 block)."""
...
def build_model():
    """Proven v2 DS-CNN (no SE) so tfmot QAT can quantize every layer."""
...
class EMACallback(keras.callbacks.Callback):
    """Maintain an exponential moving average of the model weights and
    apply them at the end of training for a small, free accuracy boost."""
    ...
    def on_train_begin(self, logs=None):
        self.ema = [w.numpy().copy() for w in self.model.weights]

    def on_train_batch_end(self, batch, logs=None):
        d = self.decay
        for i, w in enumerate(self.model.weights):
            self.ema[i] = d * self.ema[i] + (1.0 - d) * w.numpy()

    def on_train_end(self, logs=None):
        for w, e in zip(self.model.weights, self.ema):
            w.assign(e)
...
# ═══════════════════════════════════════════════════════════════════════
#  6. INT8 QUANTISATION  (from float or QAT model)
# ═══════════════════════════════════════════════════════════════════════
...
def tflite_to_c_header(tflite_data, header_path):
    ...
    with open(header_path, "w") as f:
        f.write("/* Auto-generated by train_model_v3.py — do not edit. */\n")
...
# ═══════════════════════════════════════════════════════════════════════
#  9. BUILD tf.data PIPELINE (augment + mixup)
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    ...
    print("=" * 64)
    print("  TRAFFIC SIGN RECOGNITION — MODEL TRAINING v3")
    print("  Stacked: QAT(A) + SE(B) + Focal(C) + EMA(E)")
    print("=" * 64)
    ...
    model.save(os.path.join(MODEL_DIR, "float_model.keras"))
    ...
    try:
        ...
        qat_model.compile(optimizer=keras.optimizers.Adam(QAT_LR),
                          loss=train_loss, metrics=["accuracy"])
        # QAT: no mixup (needs clean gradients), mild augmentation only
    ...
    if qat_ok:
        tflite_data = convert_to_int8(qat_model, x_train, TFLITE_PATH,
                                      from_qat=True)
    else:
